...
class RecursiveCharacterTextSplitter(TextSplitter):
    ...
    def _recurse_split(self, text: str) -> List[str]:
        ...
        inner_splitter = RecursiveCharacterTextSplitter(
            separators=self.separators[self.separators.index(self.separators[0])+1:] if len(self.separators) > 1 else [""],
...
class TextProcessor:
    ...
    def __init__(self):
        ...
        self.default_splitter = RecursiveCharacterTextSplitter(chunk_size=2000, chunk_overlap=200)
    ...
    async def map_reduce_summarize(self, text: str, query: str, llm: Any, splitter: Optional[TextSplitter] = None) -> str:
        ...
        # 并发执行 Map 阶段
        print(f"  [TextProcessor] 正在发起 {min(len(chunks), 5)} 个分块的并发摘要 (Map)...")
        ...
        print(f"  [TextProcessor] Map 阶段完成，成功产出 {len(valid_intermediates)} 份中间摘要。")
